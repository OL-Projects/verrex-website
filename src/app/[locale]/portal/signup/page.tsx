...
export default function SignupPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    // Phase 1: just show success — real registration in Phase 2
    setStep("success")
  }
        ...
        <div className="p-8 rounded-2xl bg-white/60 dark:bg-white/5 backdrop-blur-xl border border-white/40 dark:border-white/10 shadow-xl">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">
              {step === "success" ? "Request Submitted" : "Create Account"}
            </h1>
          </div>
          ...
          {step === "role" && (
            <div className="space-y-3">
                ...
                <motion.button
                  ...
                  className={`w-full flex items-center gap-4 p-4 rounded-xl border transition-all text-left cursor-pointer ${
                    selectedRole === option.value
                      ...
                      : "border-slate-200 dark:border-white/10 hover:border-blue-300 dark:hover:border-blue-500/30 bg-white/30 dark:bg-white/5"
                  }`}
                >
                  ...
                  {!option.selfSignup && (
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-500/15 text-amber-700 dark:text-amber-400 font-medium">
                      Invite Only
                    </span>
                  )}
                </motion.button>
              ...
              {/* Show request message for invite-only roles */}
              {selectedRole && !roleOptions.find(r => r.value === selectedRole)?.selfSignup && (
                ...
                >
                  ...
                  <p className="text-xs text-amber-700 dark:text-amber-400/70">
                    Contact your Verrex representative or email admin@verrex.com to request access.
                  </p>
                </motion.div>
              )}
              ...
              <div className="pt-4 text-center">
                <p className="text-sm text-slate-600 dark:text-slate-400">
                  ...
                  <IntlLink href="/portal/login" className="font-medium text-blue-600 dark:text-blue-400">
                    Log In
                  </IntlLink>
                </p>
              </div>
          ...
          {/* Step 2: Registration Form (Client only in Phase 1) */}
          {step === "form" && (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button type="submit" className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-medium flex items-center justify-center gap-2">
                <UserPlus className="h-4 w-4" />
                Create Account
              </Button>
              <button type="button" onClick={() => setStep("role")} className="w-full text-sm text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 transition-colors">
