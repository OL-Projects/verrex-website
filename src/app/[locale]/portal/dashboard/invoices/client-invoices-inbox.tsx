...
import { AlertCircle, CheckCircle2, ClipboardSignature } from "lucide-react"
...
type SuccessBanner = { type: "accepted" | "revision"; title: string } | null

export default function ClientInvoicesInbox({ clientId }: { clientId: string }) {
  const { docs, loading, markAsRead, refetch } = useClientDocuments("invoice")
  ...
  // Actions for invoices: Acknowledge & Sign, Request Changes
        ...
        <div className={`absolute top-4 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-xl shadow-lg border flex items-center gap-3 animate-in fade-in slide-in-from-top-2 duration-300 ${
          ...
          <div>
            <p className="text-sm font-semibold">
              {success.type === "accepted" && "Invoice Acknowledged & Signed"}
