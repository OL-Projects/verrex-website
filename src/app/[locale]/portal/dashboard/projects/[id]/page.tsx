...
import {
  getProjectById, getTimelineByProject, getOrdersByProject,
  getAppointmentsByProject, getMeasurementsByProject,
  getInvoicesByProject, getMessagesByProject, mockUsers,
...
import {
  ...
  FileText, Truck, Send, Lock, ChevronRight,
} from "lucide-react"
...
export default function ProjectDetailPage() {
  ...
  const timeline = getTimelineByProject(projectId, role)
  const orders = getOrdersByProject(projectId)
  const appointments = getAppointmentsByProject(projectId)
  const measurements = getMeasurementsByProject(projectId)
  const invoices = getInvoicesByProject(projectId)
  const messages = getMessagesByProject(projectId, role)
...
function OverviewTab({ project, contractor, inspector, appointments, measurements }: {
  project: ReturnType<typeof getProjectById> & {}; contractor?: { name: string; phone?: string };
  inspector?: { name: string; phone?: string }; appointments: ReturnType<typeof getAppointmentsByProject>;
  measurements: ReturnType<typeof getMeasurementsByProject>
}) {
  return (
...
function ProductsTab({ products }: { products: ReturnType<typeof getProjectById> extends undefined ? never : NonNullable<ReturnType<typeof getProjectById>>["products"] }) {
  return (
    <div className="space-y-3">
      {products.map(p => (
...
function OrdersTab({ orders }: { orders: ReturnType<typeof getOrdersByProject> }) {
...
function MessagesTab({ messages, userId }: { messages: ReturnType<typeof getMessagesByProject>; userId: string }) {
  if (messages.length === 0) return <div className="py-12 text-center"><MessageSquare className="h-10 w-10 text-slate-300 dark:text-slate-600 mx-auto mb-3" /><p className="text-sm text-slate-500">No messages in this project yet.</p></div>
  return (
...
function FinancialsTab({ project, invoices }: { project: NonNullable<ReturnType<typeof getProjectById>>; invoices: ReturnType<typeof getInvoicesByProject> }) {
  return (
    <div className="space-y-6">
